...
@Component({
  selector: 'amexio-slider', template: `
    <input type="range"
           [(ngModel)]="value"
           #inp="ngModel"
           ...
           orient="vertical"
           value="15"
           [ngStyle]="{'background-image' : '-webkit-gradient(linear, left top, right top,color-stop(' + val + ', #94A14E),color-stop(' + val + ', #C5C5C5)'}">
...
export class AmexioSliderComponent implements OnInit, ControlValueAccessor {
  ...
  @Input('step-value') stepvalue: number;
